...
#%%
class DataSet: 
    def __init__(self, vid:VideoSetReader, cfg, views_xywh, c_channel_in=1):
...
class MyWorker():
    ...
    def compute(self, args):
        ...
        vid = ffmpegcv.VideoCaptureNV(video_file, pix_fmt='rgb24', crop_xywh=canvas_xywh.tolist(), 
                                      ...
                                      gpu=self.cuda)
        vid = VideoSetReader(video_file, nvideo=len(views_xywh_np), pix_fmt='rgb24',
                             resize = feature_in_wh, resize_keepratio=False)
        
        c_channel_in=1 if vid.pix_fmt=='gray' else 3
