...
#%% 1. 【可选】从头开始，矫正内参
checkboard_move_mp4=`ls checkboard_move_cam[0-9].mp4`
# echo "$checkboard_move_mp4" | xargs -n 1 -P 0 python -m lilab.cvutils_new.extract_frames --npick 100

ncam=`ls checkboard_move_cam[0-9].mp4 | wc -l`
setupname=frank
for ((i=0; i<=$ncam-1; i++)); do
    python -m lilab.cvutils_new.extract_frames --npick 100 --iview $i --setupname $setupname checkboard_move_cam$((i+1)).mp4
...
#%% 2. 【可选】 从头开始，视频抽帧，打标
# f1.mp4 2.mp4 3.mp4
python -m lilab.cvutils_new.extract_frames_pannel_crop_random ball_move.mp4  --npick 300 --setupname $setupname
...
ls ball_frames/*.json -Sr | xargs -n 1 wc -l | head -n 20   #手动删除所有小于5行的文件
rm `comm -3 <(ls -1 ball_frames/*.jpg | sort) <(ls -1 ball_frames/*.json | sed s/json/jpg/ | sort)`
project_nake_name=ball_20240410
...
tball=" 0 0 0 0 5 " # 没有用，只是占位
# python -m lilabnext.multiview_zyy.s2_matpkl2ballpkl $vfile.matpkl  --time  $tball --force-setupname  $setupname
python -m lilab.multiview_scripts_dev.s2_matpkl2ballpkl $vfile.matpkl  --time  $tball --force-setupname  $setupname --nchoose 1000
...
python -m lilab.multiview_scripts_dev.s5_show_calibpkl2video $vfile.matcalibpkl --axis-length 0
