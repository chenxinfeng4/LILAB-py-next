...
import time
from lilabnext.multiview_calib.p2d_raymatch import ray_match_factory
...
if __name__=='__main__':
    ...
    parser.add_argument('--iins-list', nargs='*', type=int)
    ...
    ba_poses = rpc_client.ba_poses()
    ncam_localize = len(ba_poses)
    ...
    ray_match_fun = ray_match_factory(ba_poses=ba_poses, nins=rpc_client.nins())

    while True:
        iframe, p2d_shuffle = rpc_client.p2d()  #(nview, nins, 2)
        object_p = ray_match_fun(p2d_shuffle)          #(nins, 3)
